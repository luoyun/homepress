...
from ..cms.models import CMSCategory, CMSArticle
...
class NavRequestHandler(RequestHandler):
    ...
    def get_entry_url(self, form):
        ...
        elif TYPE == 2: # CMSCategory

            ID = self.get_int( form.target.data )
            C = self.db.query(CMSCategory).get( ID )
            if C:
                url = self.reverse_url('cms:category:view', C.id)
            else:
                form.target.errors.append(
                    _('Can not find category %s')  % ID )

        elif TYPE == 3: # CMSArticle

            ID = self.get_int( form.target.data )
            A = self.db.query(CMSArticle).get( ID )
            if A:
                url = self.reverse_url('cms:article:view', A.id)
            else:
                form.target.errors.append(
                    _('Can not find article %s')  % ID )
